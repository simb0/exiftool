...
public class ScanFilesQueryImplTest {
    ...
    public void testBuildResult() throws IOException {
        ...
        when(result.next()).thenReturn("CreateDate:true", "FileSize:false", "I CANNOT BE PARSED");
        ...
        assertEquals(new ArrayList<>(tags.values()).subList(1, tags.size()), Arrays.asList("true", "false"));
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testBuildResultException() throws IOException {
        Path file = Files.createTempFile("unittest", "test");
        ExecutionResult result = mock(ExecutionResult.class);
        when(result.hasNext()).thenReturn(true, true, true, true, false);
        when(result.next()).thenReturn("value3:null", "I CANNOT BE PARSED");
        doCallRealMethod().when(result).forEachRemaining(any());
