...
public enum StandardTag implements Tag {
	...
	private static final Map<String, StandardTag> ENUM_MAP;

	static {
		Map<String,StandardTag> map = new ConcurrentHashMap<>();
		for (StandardTag instance : StandardTag.values()) {
	...
	public static StandardTag fromString(String name) {
		if(!ENUM_MAP.containsKey(name)) {
            throw new IllegalArgumentException(name+" not found in EnumMap.");
		}
	...
	private enum Type {
		...
		ARRAY {
			@Override
			public <T> T parse(String value) {
				return (T) value.split(Pattern.quote(":")); }
		};
